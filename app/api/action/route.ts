    ...
    if (floor == 9) {
      ...
      return new NextResponse(
        getFrameHtml({
          ...
          image: `${process.env.NEXT_PUBLIC_URL}/api/images/action-status?floor=${floor}&gold=${gold}&hp=${playerStageStatus.hp}&attack=${playerStageStatus.attack}&defense=${playerStageStatus.defense}&weapon=${weapon}&shield=${shield}&random=${randomValue}`,
        }),
      );
    } else {
      // Normal Battle
